...
# Функция AI-агента
def p2m_agent(cpu_usage, gpu_usage, temp):
    print(f"CPU: {cpu_usage}%, GPU: {gpu_usage}%, Temp: {temp}°C")
    
    # Ограничения по температуре
    if temp > 80:
        power = 50
        print("Temp > 80°C! Reducing to 50% power.")
    elif cpu_usage > 60:  # Игра
        power = 20
        print("Gaming detected! Setting 20% power.")
    elif cpu_usage < 10:  # Простой
        power = 100
        print("Idle detected! Full 100% power.")
    else:  # Видео/браузер
        power = 70
        print("Light usage! Setting 70% power.")
    ...
    print(f"Best task: {best_task} | Hashrate: {hashrate:.2f} | Profit: ${profit:.3f}/day")
...
for scenario in scenarios:
    print("\nTesting scenario:")
    task, hashrate, profit = p2m_agent(**scenario)
